...
<stylesheet
    ...
    <variable name='this'>
        <choose>
            <when test="string-length($base)>0">
            ...
            <choose>
                ...
                <when test="ancestor::*[attribute::about or ( not(attribute::href) and ( attribute::rel or attribute::rev) )][position()=1]">
                    <variable name="selected_ancestor"
                              select="ancestor::*[attribute::about or ( not(attribute::href) and (attribute::rel or attribute::rev) )][position()=1]"/>
                    <choose>
            ...
            <choose>
                <when test="contains($string,' ')">
                    ...
                    <call-template name="tokenize">
                        <with-param name="string" select="normalize-space(substring-after($string,' '))"/>
                    </call-template>
                </when>
                <otherwise>
    ...
    <template name="self-curie-or-uri">
        ...
        <choose>
            ...
            <otherwise>blank node
                <value-of select="concat('#',generate-id($node))"/>
            </otherwise>
        </choose>
    </template>
    ...
    <template name="expand-curie-or-uri">
        ...
        <choose>
            ...
            <when test="starts-with($curie_or_uri,'[')"> <!-- we have a CURIE between square brackets -->
                <call-template name="expand-ns">
                    <with-param name="qname" select="substring-after(substring-before($curie_or_uri,']'),'[')"/>
                </call-template>
            </when>
    ...
    <template name="relation">
        ...
        <variable name="single-predicate">
            <call-template name="get-first-predicate">
        ...
        <choose>
                    ...
                    <choose>
                        <when test="starts-with($subject,'blank node ')">
                            <attribute name="rdf:nodeID">
                                <value-of select="substring-after($subject,'blank node ')"/>
                            </attribute>
                        </when>
                        <otherwise>
                    ...
                    <element name="{$single-predicate}" namespace="{$predicate-ns}">
                        <choose>
                            <when test="starts-with($object,'blank node ')">
                                <attribute name="rdf:nodeID">
                                    <value-of select="substring-after($object,'blank node ')"/>
                                </attribute>
                            </when>
                            <otherwise>
            ...
            <otherwise> <!-- generate a comment for debug -->
                <xsl:comment>Could not produce the triple for:
                    <value-of select="$subject"/> -
                    ...
                    <element name="{$single-predicate}" namespace="{$predicate-ns}">
                        ...
                        <choose>
                            ...
                            <when test="string-length($datatype)>0">
                                ...
                                <variable name="expended-datatype">
                                    <call-template name="expand-ns">
                                        <with-param name="qname" select="$datatype"/>
                                    </call-template>
                                </variable>
                                <attribute name="rdf:datatype">
                                    <value-of select="$expended-datatype"/>
                                </attribute>
                                <choose>
                                    <when test="$attrib='true'"> <!-- content is in an attribute -->
                                        <value-of select="string($object)"/>
                            ...
                            <otherwise> <!-- there is no datatype -->
                                <choose>
                                    <when test="$attrib='true'"> <!-- content is in an attribute -->
                                        <attribute name="rdf:datatype">
                                            <value-of select="'http://www.w3.org/2000/01/rdf-schema#Literal'"/>
                                        </attribute>
                                        <value-of select="string($object)"/>
                                    </when>
                                    <otherwise> <!-- content is in the element -->
                                        <attribute name="rdf:datatype">
                                            <value-of select="'http://www.w3.org/1999/02/22-rdf-syntax-ns#XMLLiteral'"/>
                                        </attribute>
